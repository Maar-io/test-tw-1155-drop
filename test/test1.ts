...
describe("Token contract", function () {
  ...
  beforeEach(async () => {
    ...
    expect(res[6]).to.equal("0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE");
  ...
  it("Evolution works", async function () {
    ...
    await gachaContract.claim(account1.address, ENERGY_TOKEN_ID, "2", "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE", 0, myAllowlistProof, "0x");
    ...
    await gachaContract.claim(account1.address, BASE_TOKEN_ID, "1", "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE", 0, myAllowlistProof, "0x");
    ...
    await gachaContract.claim(account1.address, EVOLVED_TOKEN_ID, "1", "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE", 0, myAllowlistProof, "0x");
  ...
  it("Fails to evolve, missing not enough energy tokens", async function () {
    expect(await gachaContract.totalSupply(ENERGY_TOKEN_ID)).to.equal(0);
